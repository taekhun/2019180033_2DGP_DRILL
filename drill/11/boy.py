...
class IDLE:
    ...
    def do(self):
        ...
        if self.timer == 0:  # 시간이 경과하면,
            # 이벤트를 발생시켜줘야한다. TIMER
            # self.q.insert(0, TIMER)  # 객체지향프로그래밍 위배, q를 직접 엑세스 하므로,,
...
next_state = {
    IDLE: {RU: RUN, LU: RUN, RD: RUN, LD: RUN, TIMER: SLEEP, KEY: AUTO_RUN},
    RUN: {RU: IDLE, LU: IDLE, RD: IDLE, LD: IDLE, KEY: AUTO_RUN},
    SLEEP: {RD: RUN, LD: RUN, RU: RUN, LU: RUN, SLEEP: SLEEP, KEY: SLEEP},
    AUTO_RUN: {RD: RUN, LD: RUN, RU: RUN, LU: RUN, KEY: IDLE}
}
